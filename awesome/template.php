...
//
// ZALOGA za razlicne atribute (velikost, barva, dolzina)
//
    ...
    foreach($productArray as $product) {
        ...
        $xml->startElement("Izdelek");
            ...
            $xml->startElement("Opis");
                $xml->writeRaw(trim(preg_replace('/ +/', ' ', preg_replace('/[^A-Za-z0-9 .,@!?()-]/', ' ', urldecode(html_entity_decode(strip_tags($product['description_short'])))))));
            $xml->endElement();

            $xml->startElement("ImeIzdelka");
                $xml->writeRaw(trim(preg_replace('/ +/', ' ', preg_replace('/[^A-Za-z0-9 .,@!?()-]/', ' ', urldecode(html_entity_decode(strip_tags($product['productName'])))))));
            $xml->endElement();

            // za SlikaVelika naredi, da se ponavlja kolikor je slik
            ...
            $xml->startElement("Kategorije");
                ...
                foreach($categories as $category)
                {
                    $xml->startElement("Kategorija");
                        ...
                        $xml->writeAttribute('name', trim(preg_replace('/ +/', ' ', preg_replace('/[^A-Za-z0-9 .,@!?()-]/', ' ', urldecode(html_entity_decode(strip_tags($category['categoryName'])))))));
                    $xml->endElement();
                }
                ...
                $xml->startElement("SkupinaOpcijeIzdelka");
                    ...
                    foreach($stocks as $stock)
                    {
                        $xml->startElement("OpcijaIzdelka");
                            $xml->writeAttribute('name', trim(preg_replace('/ +/', ' ', preg_replace('/[^A-Za-z0-9 .,@!?()-]/', ' ', urldecode(html_entity_decode(strip_tags($stock['attributeName'])))))));
...
/*function strip($name) {
    return trim(preg_replace('/ +/', ' ', preg_replace('/[^A-Za-z0-9 .,@!?()-]/', ' ', urldecode(html_entity_decode(strip_tags($name))))));
